...
from app import application, login_manager, admin
from app.forms import RegistrationForm, LoginForm, EditUserForm, AddGameForm, AddGroupForm
from app.models import User, Role, Team, UserAdminView, RoleAdminView, UploadFile, Game, Bet, UserTournaments, GamesPlayed, BetGroup, UserBetGroup
from app.database import db_session
from app.config import Config
from app.helpers import Helpers
from app.database_reader import TeamReader, GameReader, UserReader, TournamentReader, BetReader, UserBetGroupReader, FilesReader, BetGroupReader
from app.my_error import TeamsDatabaseEmpty, ImagesNotExist, GameNotExist, DatabaseReaderProblem, DatabaseWriterError, GamesDatabaseEmpty, BetsDatabaseEmpty
from app.database_writer import GameWriter, TeamWriter, FileWriter, UserWriter, BetWriter, TournamentWriter, BetGroupWriter, UserBetGroupWriter
...
@application.route('/edit/<int:user_id>', methods=('GET', 'POST'))
@login_required
def edit(user_id):
    form = EditUserForm()
    user = UserReader.get_one_user_filter("id", user_id)
...
@application.post('/delete/<int:user_id>')
@login_required
def delete(user_id):
...
@application.route('/admin/upload_file', methods=['GET', 'POST'])
@login_required
def upload_file():
    if request.method == 'POST':
...
@application.route('/admin/select_file', methods=['GET', 'POST'])
@login_required
def select_file():
    try:
...
@application.route('/admin/games', methods=['GET', 'POST'])
@login_required
def games():
    try:
...
@application.route('/admin/game_edit', methods=['GET', 'POST'])
@login_required
def game_edit():
    try:
        games = GameReader.get_all_games()
...
@application.post('/admin/edit_one_game/<int:game_id>')
@login_required
...
@application.post('/admin/edit_all_games')
@login_required
def edit_all_games():
    try:
...
@application.post('/admin/delete_game/<int:game_id>')
@login_required
...
@application.route('/user/bets', methods=['GET', 'POST'])
@login_required
def bets():
    try:
        user_id = current_user.get_id()
...
@application.post('/user/load_bets')
@login_required
def load_bets():
    try:
        user_id = current_user.get_id()
...
@application.post('/user/edit_bet/<int:bet_id>')
@login_required
def edit_bet(bet_id):
...
@application.route('/user/add_group', methods=('GET', 'POST'))
@login_required
def add_group():
    try:
        user_id = current_user.get_id()
...
@application.route('/user/group/<int:group_id>', methods=['GET', 'POST'])
@login_required
def group(group_id):
    try:
...
@application.post('/user/group/add_user_bet_group/<int:group_id>')
@login_required
...
@ application.route('/admin/db_update', methods=['GET', 'POST'])
@login_required
def db_update():
